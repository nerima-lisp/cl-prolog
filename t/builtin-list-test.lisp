...
(deftest cyclic-copy-helpers-preserve-cycles-and-sharing ()
  (cl-prolog::%with-logic-variable-order
    (let* ((variable (cl-prolog:fresh-logic-variable "?SOURCE"))
      ...
      (let* ((alias (cl-prolog:fresh-logic-variable "?ALIAS"))
             (alias-target (cl-prolog:fresh-logic-variable "?ALIAS-TARGET"))
             (residual (cl-prolog:fresh-logic-variable "?RESIDUAL"))
        ...
        (multiple-value-bind (legacy legacy-cyclic-p legacy-variables-p)
            (cl-prolog::%canonicalize-variant
             (cl-prolog::%logic-substitute-indexed root index))
          (multiple-value-bind (fused fused-cyclic-p fused-variables-p)
              (cl-prolog::%canonicalize-variant root index)
            (is-equal (cl-prolog::%variant-graph-key legacy)
                      (cl-prolog::%variant-graph-key fused))
...
(deftest variant-canonicalization-preserves-graph-across-copy-map-promotion ()
  (let* ((capacity 16)
    ...
    (multiple-value-bind (canonical cyclic-p contains-variables-p)
        (cl-prolog::%canonicalize-variant root)
      ...
      (let ((canonical-last
        ...
        (is (eq (car canonical)
                (car canonical-last)))))))
(deftest variant-canonicalization-four-cons-backedge-preserves-copy-root ()
  (let* ((root (make-list 4))
         (last (last root)))
    (setf (cdr last) root)
    (multiple-value-bind (canonical cyclic-p contains-variables-p)
        (cl-prolog::%canonicalize-variant root)
      ...
      (is (not (eq canonical root)))
      (is cyclic-p)
      (is (eq canonical (nthcdr 4 canonical))))))
(deftest variant-canonicalization-five-cons-promotion-backedge-preserves-copy-root ()
  (let* ((root (make-list 5))
         (last (last root)))
    (setf (cdr last) root)
    (multiple-value-bind (canonical cyclic-p contains-variables-p)
        (cl-prolog::%canonicalize-variant root)
      ...
      (is (not (eq canonical root)))
      (is cyclic-p)
      (is (eq canonical (nthcdr 5 canonical))))))
(deftest variant-canonicalization-post-promotion-shared-dag-stays-acyclic ()
  (let* ((variable (cl-prolog:fresh-logic-variable "?SHARED"))
