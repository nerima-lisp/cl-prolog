...
(deftest occurs-check-default-true-fails-on-cycle ()
  (let ((rulebase (make-rulebase)))
    (assert-query rulebase (= ?x (cl-prolog.user-atoms::f ?x)) :fails)
    (assert-query rulebase
                  (cl-prolog::unify_with_occurs_check ?x (cl-prolog.user-atoms::f ?x))
                  :fails)
    (assert-query rulebase (= ?x hello) :succeeds)))

(deftest occurs-check-false-allows-cycle ()
  (let ((rulebase (make-rulebase)))
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                            cl-prolog.user-atoms::false)
                  :succeeds)
    ;; X = f(X) now succeeds (creating a cyclic term); assert success only.
    (assert-query rulebase (= ?x (cl-prolog.user-atoms::f ?x)) :succeeds)
...
(deftest occurs-check-error-raises-on-cycle ()
  (let ((rulebase (make-rulebase)))
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                            cl-prolog.user-atoms::error)
                  :succeeds)
    (assert-query rulebase (= ?x (cl-prolog.user-atoms::f ?x)) :signals)
...
(deftest occurs-check-default-nested-cycle ()
  (let ((rulebase (make-rulebase)))
    ...
    (assert-query rulebase (= ?x (cl-prolog.user-atoms::s a ?x)) :fails)))
...
(deftest cyclic-term-writer-terminates ()
  ...
  (with-io-rulebase (rulebase input output) ""
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                                       cl-prolog.user-atoms::false)
                  :succeeds)
    (assert-query rulebase (and (= ?x (cl-prolog.user-atoms::f ?x))
                                (cl-prolog::write ?x))
                  :succeeds)
  ...
  (with-io-rulebase (rulebase input output) ""
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                                       cl-prolog.user-atoms::false)
                  :succeeds)
    (assert-query rulebase (and (= ?x (cl-prolog.user-atoms::s a ?x))
                                (cl-prolog::write ?x))
                  :succeeds)
  ...
  (with-io-rulebase (rulebase input output) ""
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                                       cl-prolog.user-atoms::false)
                  :succeeds)
    (assert-query rulebase (and (= ?x (a . ?x)) (cl-prolog::write ?x))
                  :succeeds)
...
(deftest acyclic-shared-subterm-prints-in-full ()
  ...
  (with-io-rulebase (rulebase input output) ""
    (assert-query rulebase (and (= ?s (cl-prolog.user-atoms::f a))
                                (cl-prolog::write (cl-prolog.user-atoms::pair ?s ?s)))
                  :succeeds)
...
(deftest occurs-check-negation-honors-flag ()
  ...
  (let ((rulebase (make-rulebase)))
    (assert-query rulebase (cl-prolog:|\\=| ?x (cl-prolog.user-atoms::f ?x)) :succeeds))
  (let ((rulebase (make-rulebase)))
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                            cl-prolog.user-atoms::false)
                  :succeeds)
    (assert-query rulebase (cl-prolog:|\\=| ?x (cl-prolog.user-atoms::f ?x)) :fails))
  (let ((rulebase (make-rulebase)))
    (assert-query rulebase (cl-prolog::set_prolog_flag cl-prolog.user-atoms::occurs_check
                                            cl-prolog.user-atoms::error)
                  :succeeds)
    (assert-query rulebase (cl-prolog:|\\=| ?x (cl-prolog.user-atoms::f ?x)) :signals)))
